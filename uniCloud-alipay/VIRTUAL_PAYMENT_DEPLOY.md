...
真单验证时购买当前 8 元的 `membership_1m`，确认 `question_bank_payment_orders.status` 为 `delivered`、`question_bank_memberships.expiresAt` 延长 31 天，并验证免广告与云端学习数据同步权益。错题集、收藏夹、考试模式和背题模式对所有用户开放。自然到期允许最多 6 小时宽限，普通页面的会员状态缓存同样为 6 小时；会员中心始终强制刷新。
