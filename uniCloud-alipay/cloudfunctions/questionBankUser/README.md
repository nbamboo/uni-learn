...
- `syncEvents`：批量同步答题、收藏和最近学习进度；收藏写入要求有效会员。新版客户端直接上传本地判题结果、章节和知识点，云端不再读取题库目录或题目答案；响应中的 `summaries` 返回本批次涉及科目的最新统计。旧版答题事件在发布过渡期内仍兼容云端判题。
...
- `getRecords`：分页读取错题集或收藏夹；两者均要求有效会员。仅第一页统计总数，后续页通过多取一条判断 `hasMore`。
- `getUserProfile`：读取当前登录用户的安全资料摘要，仅返回 UID、昵称、头像、微信绑定状态和时间信息。
- `getPreferences`：读取当前用户的答题模式和夜间模式；会员过期后考试/背题模式自动降级为做题模式。
- `updatePreferences`：校验并覆盖当前用户的答题模式和夜间模式；考试/背题模式要求有效会员，用户 ID 只取自已验证 token。
