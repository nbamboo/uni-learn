...
客户端普通页面缓存会员状态 6 小时；当本地状态仍为会员时，进入会员专属功能前会强制调用 `getMembership`。退款回调会立即撤销对应授权，后台人工撤销必须将会员状态设为 `revoked`；两者都会在下一次权益校验时覆盖客户端本地缓存。所有会员用户数据 action 仍由 `questionBankUser` 在服务端独立校验，不能依赖客户端缓存绕过。
